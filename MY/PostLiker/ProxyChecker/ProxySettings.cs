...
namespace PostLiker
{
    public partial class ProxySettings : Form
    {
        ...
        private void CheckProxy(object sender, EventArgs e)
        {
            ...
            {
                ProxyChecker PC = new ProxyChecker("https://google.com/", proxy, this.outputProxy, new Action(() =>
                {
                    this.progressBar.Value++;
                    if (this.progressBar.Value == this.progressBar.Maximum)
                        this.checkProxyButton.Enabled = true;
                }));
                ThreadPool.QueueUserWorkItem(PC.Check, null);
            }
        }
    ...
    class ProxyChecker
    {
        ...
        public ProxyChecker(String url, String proxy, ListBox proxy_list, Action valueChanged)
        {
            this.proxy = proxy;
            this.url = url;
