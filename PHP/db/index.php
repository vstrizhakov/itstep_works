<?php
	...
	{
		$cs = $category = StringOrNullFromArray("category", $_POST);
		$ts = $theme = StringOrNullFromArray("theme", $_POST);
		$ps = $publication = StringOrNullFromArray("publication", $_POST);
		...
		<div class="container my-4">
			...
			<form class="row my-4" method="POST">
				<div class="col col-lg-3">
					<h4>Категория:</h4>
					<select name="category" id="" class="custom-select custom-select-lg mb-3">
				...
				<div class="col col-lg-3">
					<h4>Тематика:</h4>
					<select name="theme" id="" class="custom-select custom-select-lg mb-3">
						<option value="None" selected>Не выбрано...</option>
				...
				<div class="col col-lg-3">
					<h4>Издательство:</h4>
					<select name="publication" id="" class="custom-select custom-select-lg mb-3">
						<option value="None" selected>Не выбрано...</option>
				...
				<div class="col col-lg-3 d-flex align-items-end mb-3">
					<button class="btn btn-primary btn-block btn-lg" name="submit">Поиск</button>
				</div>
			</form>
					...
					<tbody>
						<?php foreach ($books as $book) { ?>
							<tr>
								...
								<th><?= $book['themes']; ?></th>
								<th><?= $book['izd']; ?></th>
								<th><?= number_format($book['price'], 2, '.', ''); ?></th>
							</tr>
						<?php } ?>
					</tbody>
				</table>
			<?php } else { ?>
				<h3 class="text-center">Книг по заданному запросу не найдено!</h3>
			<?php } ?>
		</div>
	</body>
</html>