...
class CTovarControllersTovar extends JControllerAdmin
{
    ...
    function edit()
    {
       ...
       try
       {
           // ------------- Проверка, что элемент выбран для редактирования
            ...
            if($id=='')
                throw new Exception("Не выбран товар для редактирования");
    ...
    function remove()
    {
        ...
        {
            $app->input->set('task','');
            $app->enqueueMessage("Не выбран товар для удаления", 'error');
        }
        else{
            // --- Удаление
            ...
            $db->setQuery($query);
            $db->execute();
            $app->enqueueMessage("Товар удален успешно", "message");
        }
        $this->display();
    }
    function display($cachhable = false, $urlparams = array()) // показывает контроллер (его страницу)
    {
       // echo "<h1 style='color: blue'>Товары</h1>";
           ...
           // ---------- Заголовок, кнопки
           JToolBarHelper::title('Компонент для работы с продуктами CTovar : Товары');
