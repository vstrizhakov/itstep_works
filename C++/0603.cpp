...
void main() {
	...
	days[0] = (((ivan.hireday.year * 12) + ivan.hireday.month) * 30) + ivan.hireday.day;
	days[1] = (((sergey.hireday.year * 12) + sergey.hireday.month) * 30) + sergey.hireday.day;
	days[2] = (((petya.hireday.year * 12) + petya.hireday.month) * 30) + petya.hireday.day;
	if (days[0] >= days[1] && days[0] >= days[2]) {
		min_year = ivan.hireday.year;
		min_month = ivan.hireday.month;
		min_day = ivan.hireday.day;
	}
	if (days[1] >= days[0] && days[1] >= days[2]) {
		min_year = sergey.hireday.year;
		min_month = sergey.hireday.month;
		min_day = sergey.hireday.day;
	}
	if (days[2] >= days[1] && days[2] >= days[0]) {
	...
	while (i > 0) {
		...
		do {
			cout << "Введите день(1-30): ";
			cin >> n_day;
		}  while (n_day < 1 || n_day > 30);
		do {
			cout << "Введите месяц(1-12): ";
			cin >> n_month;
		} while (n_month < 1 || n_month > 12);
		do {
			cout << "Введите год(больше 0): ";
			cin >> n_year;
		} while (n_year < 0);
	...
	month = n_month - (ivan.hireday.month);
	if (day < 0) {
		day = 30 + day;
		month--;
	}
	...
	month = n_month - (sergey.hireday.month);
	if (day < 0) {
		day = 30 + day;
		month--;
	}
	...
	month = n_month - (petya.hireday.month);
	if (day < 0) {
		day = 30 + day;
		month--;
	}
	...
	else if (days[1] == days[2] && days[1] > days[0]) sum = 4;
	else if (days[0] == days[2] && days[2] > days[1]) sum = 5;
	else if (days[0] == days[1] && days[0] < days[2]) sum = 6;
	else if (days[1] == days[2] && days[1] < days[0]) sum = 7;
	else if (days[0] == days[2] && days[2] < days[1]) sum = 8;
	else if (days[0] == days[1] && days[1] == days[2]) sum = 9;
	if (sum == 0) cout << "Иван";
	else if (sum == 1) cout << "Сергей";
	else if (sum == 2) cout << "Петя";
	else if (sum == 3) cout << "У Ивана и Сергея одиннаковый стаж работы и он больше, чем у Пети";
	else if (sum == 4) cout << "У Сергея и Пети одиннаковый стаж работы и он больше, чем у Ивана";
	else if (sum == 5) cout << "У Ивана и Пети одиннаковый стаж работы и он больше, чем у Сергея";
	else if (sum == 6) cout << "У Ивана и Сергея одиннаковый стаж работы и он меньше, чем у Пети";
	else if (sum == 7) cout << "У Сергея и Пети одиннаковый стаж работы и он меньше, чем у Ивана";
	else if (sum == 8) cout << "У Ивана и Пети одиннаковый стаж работы и он меньше, чем у Сергея";
	else if (sum == 9) cout << "У Ивана, Сергея и Пети одинаковый стаж работы";
	if (sum > 2) cout << endl;
	if(sum >= 0 && sum < 3) cout << " имеет наибольший стаж работы" << endl;
	SetColor(15, 0);
	cout << "---------------------------------------------------------------------------------------------------\n";
}
